...
> Fecha de última actualización: 26 de febrero de 2026 (revisión 2).
...
Los **templates de acceso** (`AccessTemplate`) representan perfiles reutilizables (ej. "Vendedor — Sucursal estándar"): encapsulan un conjunto de selecciones previamente configuradas y permiten pre-rellenar nuevas solicitudes.
...
| 1 | `templates/new/companies/` | `TemplateWizardStep1CompaniesView` | Multi-selección de `Company` + sucursales + `same_modules_for_all`. Crea `PermissionSelectionSet` + `AccessTemplateItem` por empresa. |
| 2 | `templates/new/modules/` | `TemplateWizardStep2ModulesView` | Árbol de módulos (GLOBAL o PER_ITEM). Reutiliza `Step3ModulesForm` y `build_module_tree`. |
| 3 | `templates/new/globals/` | `TemplateWizardStep3GlobalsView` | Formsets de acciones, matriz CRUD, medios de pago. Reutiliza `ActionValueFormSet`, `MatrixFormSet`, `PaymentFormSet`. |
| 4 | `templates/new/scoped/` | `TemplateWizardStep4ScopedView` | Paneles/vendedores por empresa, depósitos/cajas por sucursal. Crea items de sucursal via `clone_selection_set`. |
| 5 | `templates/new/review/` | `TemplateWizardStep5ReviewView` | Vista de revisión completa (mismo payload que detalle). Al confirmar: `tmpl.is_active = True`, limpia sesión, redirige a `template_detail`. |

**Patrón draft**: el `AccessTemplate` se crea con `is_active=False` en el paso 0 para que todos los `PermissionSelectionSet`/`AccessTemplateItem` tengan FK real desde el inicio. Se activa recién en el paso 5.

**Archivos clave:**
- Vistas: `views/template_wizard/` — `base.py`, `step_0_start.py` … `step_5_review.py`
- Base class: `TemplateWizardBaseView` en `base.py` — session key `TEMPLATE_WIZARD_SESSION_KEY = "catalog_template_wizard"`, método `wizard_context(**extra)`
...
catalog:template_wizard_companies → templates/new/companies/
catalog:template_wizard_modules   → templates/new/modules/
catalog:template_wizard_globals   → templates/new/globals/
catalog:template_wizard_scoped    → templates/new/scoped/
